...
export default function EditInvoicePage() {
  ...
  const fetchData = async () => {
    try {
      ...
      const invoiceRes = await api.get(`/invoices/${params.id}`)
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true)
    try {
      const res = await api.put(`/invoices/${params.id}`, {
      ...
      showToast(err.response?.data?.message || 'Gagal mengupdate invoice', 'error')
    } finally {
      setLoading(false)
    }
  }
    ...
    <>
      <div className="flex items-center gap-4 mb-8">
        ...
        <div>
          ...
          <p className="text-gray-500 mt-1">Perbarui detail invoice</p>
        </div>
      </div>
        ...
        <>
          ...
          <div className="bg-blue-50 border border-blue-100 rounded-xl p-4 mb-6 flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-xs text-blue-500 font-medium">Invoice Number</p>
          ...
          <form onSubmit={handleSubmit} className="space-y-6 max-w-3xl">
            ...
            <div className="bg-white rounded-2xl border border-gray-100 p-6 space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">Pajak ({currencySymbols[settings.currency] || 'Rp'})</label>
                  <input
                    type="number" min="0"
                    value={form.tax}
              ...
              <div className="border-t border-gray-100 pt-4 space-y-2">
                ...
                <div className="flex justify-between text-sm text-gray-500">
                  <span>Pajak</span>
                  <span>{formatCurrency(Number(form.tax))}</span>
                </div>
                <div className="flex justify-between text-sm text-gray-500">
                  <span>Diskon</span>
